...
def rate_limit(func):
    """Decorator for rate limiting email operations"""
    @wraps(func)
    async def wrapper(*args, **kwargs):
        # Get email service from first argument (self)
        email_service = args[0].email_service if args else None
        if not email_service or not email_service.check_rate_limit():
...
def retry_on_failure(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        config = args[0].config if args else None
        if not config:
            return await func(*args, **kwargs)
...
class Config:
    ...
    def __init__(self):
        ...
        self.email_rate_limit = 60  # seconds between emails
        self.max_retries = 3
...
class CacheManager:
    """Manages caching for frequently used operations"""
...
class EmailService:
    """Handles email-related operations with rate limiting and error handling"""
    ...
    def check_rate_limit(self) -> bool:
        ...
        if current_time - self._last_email_time < self.config.email_rate_limit:
    ...
    def get_service(self):
        """Get Gmail service with proper error handling"""
        try:
        ...
        try:
            ...
            if os.path.exists(self.config.token_file):
                with open(self.config.token_file, 'r') as token:
                    creds = Credentials.from_authorized_user_info(json.load(token), self.config.gmail_scopes)
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    if not os.path.exists(self.config.client_secrets_file):
                        raise SecurityError("Missing client secrets file")
                    
                    flow = InstalledAppFlow.from_client_secrets_file(
                        self.config.client_secrets_file, self.config.gmail_scopes)
...
service_locator.register('email_service', EmailService(service_locator.get('config'), service_locator.get_cache()))
service_locator.register('cache', service_locator.get_cache())
...
config = Config()
email_service = EmailService(config, service_locator.get('cache'))
...
def add(a: int, b: int) -> int:
    """Add two numbers together with caching."""
...
def multiply(a: int, b: int) -> int:
    """Multiply two numbers together with caching."""
...
def power(a: int, b: int) -> int:
    """Calculate a raised to the power of b with caching."""
    logger.debug(f"Calculating {a} to the power of {b}")
    return int(a ** b)
...
class EmailTool:
    ...
    async def send_email(to_email: str, subject: str, body: str, image_path: str = None) -> dict:
        """
        Send an email with dependency injection and caching.
        ...
        Returns:
            dict: Response containing success status and message ID
        """
        try:
            logger.info(f"Preparing to send email to {to_email}")
            
            # Get services from locator
            email_service = service_locator.get('email_service')
            cache = service_locator.get('cache')
            
            # Check cache for recent email to same recipient
            cache_key = f"email_{to_email}_{hash(subject + body)}"
            cached_result = cache.get(cache_key)
            if cached_result:
                logger.info("Using cached email result")
                return cached_result
            
            # Validate parameters
            if not all([to_email, subject, body]):
                raise SecurityError("Missing required email parameters")
            ...
            logger.info(f"Email sent successfully with ID: {message['id']}")
            
            # Cache the successful result
            result = JsonValidator.create_response(
                is_valid=True,
